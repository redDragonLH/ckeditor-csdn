...
( function() {
	...
	CKEDITOR.plugins.enterkey = {
		enterBlock: function( editor, mode, range, forceMode ) {
			...
			var $activeDOM = $(range.endContainer.$);
			var $body = $document.find('body');
			var $bodyheight = $body.height();
			var $boxdom = $('#cke_1_contents');
			if($activeDOM[0].nodeName === 'P' && (Math.ceil($activeDOM.offset().top)+18 >= Math.ceil($body.outerHeight()) ) && (Math.ceil($body.outerHeight()) > $boxdom.height())){
				if($activeDOM.next().length === 0){
					$body.height( $activeDOM.offset().top+ ($('.cke_wysiwyg_frame ').height() / 2 ));
					$document.scrollTop($activeDOM.offset().top+($('.cke_wysiwyg_frame ').height() / 2 ))
				}
			}
