...
( function() {
	...
	function saveStyles( element, isInsideEditor,editor) {
		...
		if ( !isInsideEditor ) {
			// console.log(editor.config.saveClass);
			var nodeName = $element.nodeName;
			if(!editor.config.saveClass[nodeName]){
	...
	CKEDITOR.plugins.add( 'maximize', {
		...
		init: function( editor ) {
			...
			function setMaximizeHead(parentElement,referenceElement){
				addStyles ()

				var headHTML = '<div id="MaximizeHeadBox" class="head">'+
				'<div id="MaximizeHead">'+
				'<input type="text" id="txtTitleM" maxlength="100" placeholder="输入文章标题">'+
				'<input id="btnPublishM" type="button" class="btn btn-outline-danger btn-fabu" value="发布博客" title="发布博客">'+
				'</div></div>';
				var newElement = CKEDITOR.dom.element.createFromHtml( headHTML ).$;
				var parent = parentElement.$;
				csdn ? parent.insertBefore(newElement,referenceElement.$) : '';
					try {
						document.getElementById('txtTitleM').onchange=function(e){
						document.getElementById('txtTitle').value = e.target.value;
						}
					} catch (e) {
						console.log(e);
					}
			}
			function setMaximizeStyle(){
				// iframe 内部 body 样式
				...
				// 外部背景色
				outer = editor.container.getFirst( function( node ) {
				...
				try {
					var MaximizeHead = document.getElementById('MaximizeHeadBox');
					
					if(!MaximizeHead){
						setMaximizeHead(outer,top)
					}else{
						MaximizeHead.setAttribute('class','HeadShow')
					}
			...
			function SyncInput( inputDOMId, valueDOMId ){
				try {
					var inputDOM = document.getElementById(inputDOMId);
					var valueDOM = document.getElementById(valueDOMId);
					if(!!valueDOM && !!inputDOM){
						inputDOM.value = valueDOM.value
					}else{
			...
			function MaximizeSyncInput(inputDOMId, valueDOMId){
				SyncInput( inputDOMId, valueDOMId )
			}
			function quitMaximizeSyncInput(inputDOMId, valueDOMId){
				SyncInput( inputDOMId, valueDOMId )
			}
			function removeMaximizeStyle(){
				var MaximizeHead = document.getElementById('MaximizeHeadBox');
				if(MaximizeHead){
					MaximizeHead.setAttribute('class','HeadHide');
					var element = editor.document.getBody();
					element.setStyles({
						width:'auto',
						margin: '24px',
			...
			editor.addCommand( 'maximize', {
				...
				exec: function() {
					...
					if ( this.state == CKEDITOR.TRISTATE_OFF ) {
						...
						setMaximizeStyle()
						try {
							MaximizeSyncInput('txtTitleM', 'txtTitle')
						...
						while ( ( currentNode = currentNode.getParent() ) ) {
							currentNode.setCustomData( 'maximize_saved_styles', saveStyles( currentNode , null, editor ) );
							// Show under floatpanels (-1) and context menu (-2).
							currentNode.setStyle( 'z-index', editor.config.baseFloatZIndex - 5 );
						}
						contents.setCustomData( 'maximize_saved_styles', saveStyles( contents, true, editor ) );
						container.setCustomData( 'maximize_saved_styles', saveStyles( container, true, editor ) );
					...
					// Restore from fullscreen if the state is on.
						...
						try {
							...
							MaximizeSyncInput('txtTitle', 'txtTitleM')
