				...
				editor.addCommand( 'insertToc', {
					...
					exec: function( editor )
					{
						...
						for ( var i = 0 ; i < nodes.count() ; i++ )
						{
							...
							{
								id = encodeURIComponent(text.getText());
							...
							tocItems = tocItems + '<p style="margin-left:'+level*40+'px" id="' + id.toString() + '-toc" name="tableOfContents">' + '<a target="_self" href="#' + id.toString() + '">' + text.getText().toString() + '</a></p>';
						}

						//output toc
						var tocNode = '<p name="tableOfContents" id="main-toc"><b>' + editor.lang.toc.ToC + '</b></p>' + tocItems + '<hr id="hr-toc" name="tableOfContents"/>';
	
						editor.insertHtml(tocNode,'unfiltered_html');
