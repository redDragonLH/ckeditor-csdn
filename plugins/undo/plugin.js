...
// -------------以下是撤销/重做插件的主要逻辑
	...
	var UndoManager = CKEDITOR.plugins.undo.UndoManager = function( editor ) {
		 ...
		 */
		 ...
		 /**
		 *存储按键次数的数组，连续计数。 使用{@link #keyGroups}成员作为索引。
		 *
		 ...
		 */
		 ...
		 /**
		 *当`locked`属性不是`null`时，撤消管理器被锁定，所以
		 *诸如`save`或`update`之类的操作是被禁止的。
		 *
		 *管理员可以通过{@link #lock}和{@link #unlock}进行锁定和解锁
		 *方法，分别。
		 *
		 * @只读
		 * @property {Object} [locked = null]
	...
	UndoManager.prototype = {
		...
		type: function( keyCode, strokesPerSnapshotExceeded ) {
			var keyGroup = UndoManager.getKeyGroup( keyCode ),
				...
				//当前行中的击键次数。
				//注意，如果strokesPerSnapshotExceeded将被超出，它将被重新启动。
		 ...
		 *
		 * 新的`keyCode`是否属于与前一个不同的组（{@link #previousKeyGroup}）
		 * 
		 * @since 4.4.5
		 ...
		 *保存文档的快照供以后检索。
		 *
		 * @param {Boolean} onContentOnly如果设置为“true”，则仅当内容已更改时才会保存快照。
		 * @param {CKEDITOR.plugins.undo.Image}快照 可选 快照进行保存。 如果跳过，将使用当前编辑器。
		 * @param {Boolean} [autoFireChange = true] 如果设置为`false`，则不会触发编辑器的{@link CKEDITOR.editor＃change}事件。
		 */
		save: function( onContentOnly, image, autoFireChange ) {
			...
			// 
			//锁定时不要更改快照堆栈，编辑器未准备好，
			//编辑状态没有准备好，或者编辑器与“wysiwyg”模式不同时。
		 ...
		 *
		 * @参数{CKEDITOR.plugins.undo.Image} [newImage]将替换当前的图像。
		 * 如果未设置，则默认为从编辑器获取的图像。
		 */
	 ...
	 *
	 * 封装所有必须使用的本地事件侦听器的类
	 * 为了处理本机编辑操作的撤销管理器集成（不包括由Clipboard插件处理的拖放和粘贴支持）。
	 * @since 4.4.4
	 * @private
